/**
 * Unit tests for FormManager (SOLID refactored)
 */
...
describe('FormManager', () => {
  ...
  beforeEach(() => {
    ...
    // Création du DOM pour le formulaire
    const form = document.createElement('form');
    form.id = SELECTORS.MOOD_FORM;
    document.body.appendChild(form);

    const textInput = document.createElement('input');
    textInput.id = SELECTORS.TEXT_INPUT;
    textInput.value = 'Bonne journée';
    document.body.appendChild(textInput);

    const ratingInput = document.createElement('input');
    ratingInput.id = SELECTORS.RATING_INPUT;
    ratingInput.value = '4';
    document.body.appendChild(ratingInput);

    // Mock global fetch et alert
  ...
  test('should send mood data to backend', async () => {
